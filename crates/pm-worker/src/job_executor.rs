...
use pm_agent_client::{
    reconnect_with_backoff, types::ApplyPatchesRequest, AgentClient, InstallUrlRequest,
};
...
async fn execute_host_job(
    ...
    // ── 0. Dispatch based on job kind ────────────────────────────────────────
    let job_kind: (JobKind,) = match sqlx::query_as("SELECT kind FROM patch_jobs WHERE id = $1")
        .bind(job_id)
        .fetch_optional(&pool)
        .await
    {
        Ok(Some(row)) => row,
        Ok(None) => {
            tracing::error!(%job_id, "execute_host_job: job not found");
            handle_host_failure(pool, pjh_id, format!("Job {job_id} not found")).await;
            return;
        },
        Err(e) => {
            tracing::error!(%job_id, error = %e, "execute_host_job: DB error fetching job kind");
            handle_host_failure(pool, pjh_id, format!("DB error fetching job kind: {e}")).await;
            return;
        },
    };

    if job_kind.0 == JobKind::SelfUpgrade {
        execute_self_upgrade_host_job(pool, config, job_id, host_id, pjh_id).await;
        return;
    }
...
/// Self-upgrade orchestration: install a new version from URL, restart the
/// agent, reconnect, and verify the upgraded version.
async fn execute_self_upgrade_host_job(
    ...
    tracing::info!(%job_id, %host_id, %pjh_id, "execute_self_upgrade_host_job: starting");
    ...
    .await
    {
        Ok(Some(h)) => h,
        Ok(None) => {
            tracing::error!(%host_id, "execute_self_upgrade_host_job: host not found");
            handle_host_failure(
                pool,
                pjh_id,
                format!("Host {host_id} not found in database"),
        ...
        Err(e) => {
            tracing::error!(%host_id, error = %e, "execute_self_upgrade_host_job: DB error fetching host");
            handle_host_failure(pool, pjh_id, format!("DB error fetching host: {e}")).await;
            return;
        },
    };

    // ── 2. Fetch upgrade details from the pjh row ────────────────────────────
    #[derive(Debug, FromRow)]
    struct UpgradeRow {
        upgrade_url: Option<String>,
        upgrade_checksum: Option<String>,
        upgrade_version: Option<String>,
    }

    let upgrade: UpgradeRow = match sqlx::query_as(
        r#"
        SELECT upgrade_url, upgrade_checksum, upgrade_version
        FROM   patch_job_hosts
        WHERE  id = $1
        "#,
    )
    ...
    .await
    {
        Ok(Some(r)) => r,
        Ok(None) => {
            tracing::error!(%pjh_id, "execute_self_upgrade_host_job: pjh row not found");
            handle_host_failure(pool, pjh_id, "Patch job host row not found".to_string()).await;
            return;
        },
        Err(e) => {
            tracing::error!(%pjh_id, error = %e, "execute_self_upgrade_host_job: DB error fetching upgrade row");
            handle_host_failure(pool, pjh_id, format!("DB error fetching upgrade row: {e}")).await;
            return;
        },
    };

    let upgrade_url = match upgrade.upgrade_url {
        Some(ref u) if !u.is_empty() => u.clone(),
        _ => {
            tracing::error!(%pjh_id, "execute_self_upgrade_host_job: upgrade_url is missing");
            handle_host_failure(
                pool,
                pjh_id,
                "Missing upgrade_url on self_upgrade job".to_string(),
            )
            .await;
            return;
        },
    ...
    tracing::info!(
        %pjh_id,
        %upgrade_url,
        upgrade_checksum = ?upgrade.upgrade_checksum,
        %target_version,
        "execute_self_upgrade_host_job: upgrade details loaded"
    );
    ...
    let certs = match load_agent_certs(&config.security) {
        Ok(c) => c,
        Err(e) => {
            tracing::error!(%host_id, error = %e, "execute_self_upgrade_host_job: failed to load agent certs");
            handle_host_failure(pool, pjh_id, format!("Failed to load agent certs: {e}")).await;
            return;
        },
    };
    ...
    let client = match AgentClient::new(
        ...
        Ok(c) => c,
        Err(e) => {
            tracing::error!(%host_id, error = %e, "execute_self_upgrade_host_job: failed to build AgentClient");
            handle_host_failure(pool, pjh_id, format!("Failed to build agent client: {e}")).await;
            return;
        },
    };
    ...
    .await
    {
        tracing::error!(%pjh_id, error = %e, "execute_self_upgrade_host_job: failed to mark pjh running");
    }

    // ── 6. Submit install-url job to the agent ────────────────────────────────
    let req = InstallUrlRequest {
        url: upgrade_url,
        checksum: upgrade.upgrade_checksum,
    };

    let install_resp = match client.install_url(&req).await {
        Ok(resp) => {
            tracing::info!(
                %pjh_id,
                agent_job_id = %resp.job_id,
                "execute_self_upgrade_host_job: agent accepted install-url job"
            );
            resp
        },
        Err(e) => {
            tracing::warn!(%pjh_id, error = %e, "execute_self_upgrade_host_job: agent rejected install-url job");
            handle_host_failure(pool, pjh_id, format!("Agent install-url error: {e}")).await;
            return;
        },
    };

    // ── 7. Store agent_job_id ────────────────────────────────────────────────
    if let Err(e) = sqlx::query("UPDATE patch_job_hosts SET agent_job_id = $1 WHERE id = $2")
        .bind(&install_resp.job_id)
        .bind(pjh_id)
        .execute(&pool)
        .await
    {
    ...
    // ── 8. Poll install job until complete ────────────────────────────────────
    // We poll inline because we must restart the agent immediately after
    // the install succeeds — the periodic scanner cannot handle that.
    ...
    loop {
        if start.elapsed() >= max_poll_duration {
            tracing::error!(%pjh_id, "execute_self_upgrade_host_job: install timed out after 600s");
            handle_host_failure(pool, pjh_id, "Install job timed out".to_string()).await;
            return;
        }

        tokio::time::sleep(poll_interval).await;

        match client.job_status(&install_resp.job_id).await {
            Ok(status) => match status.status.as_str() {
                "succeeded" | "completed" => {
                    tracing::info!(%pjh_id, "execute_self_upgrade_host_job: install job succeeded");
                    install_output = status.output.unwrap_or_default();
                    break;
                },
                "failed" => {
                    let err_msg = status
                        .error
                        .unwrap_or_else(|| "Agent install job failed".to_string());
                    tracing::warn!(%pjh_id, error = %err_msg, "execute_self_upgrade_host_job: install job failed");
                    handle_host_failure(pool, pjh_id, err_msg).await;
                    return;
                },
                "cancelled" => {
                    let err_msg = status
                        .error
                        .unwrap_or_else(|| "Agent install job was cancelled".to_string());
                    tracing::warn!(%pjh_id, "execute_self_upgrade_host_job: install job cancelled");
                    handle_host_failure(pool, pjh_id, err_msg).await;
                    return;
                },
                "running" | "queued" => {
                    tracing::debug!(%pjh_id, agent_status = %status.status, "execute_self_upgrade_host_job: install in progress");
                    // Continue polling
                },
                other => {
                    tracing::warn!(%pjh_id, %other, "execute_self_upgrade_host_job: unexpected install status — continuing to poll");
                },
            },
            Err(e) => {
                // Transient errors — keep polling unless we're past the deadline.
                tracing::warn!(%pjh_id, error = %e, "execute_self_upgrade_host_job: poll error, retrying");
            },
        }
    }

    // ── 9. Restart the system ─────────────────────────────────────────────────
    tracing::info!(%pjh_id, "execute_self_upgrade_host_job: requesting system restart");
    if let Err(e) = client.restart_system().await {
        tracing::warn!(%pjh_id, error = %e, "execute_self_upgrade_host_job: restart_system call failed");
        // The agent may have already restarted or the connection dropped —
        // we still try to reconnect below.
    }

    // ── 10. Reconnect with backoff ───────────────────────────────────────────
    tracing::info!(%pjh_id, "execute_self_upgrade_host_job: waiting for agent to come back online");
    let sys_info = match reconnect_with_backoff(&client, 60).await {
        Ok(info) => info,
        Err(e) => {
            tracing::error!(%pjh_id, error = %e, "execute_self_upgrade_host_job: agent did not come back online");
            handle_host_failure(
                pool,
                pjh_id,
                format!("Agent did not reconnect within 60s: {e}"),
            )
            .await;
            return;
        },
    ...
    // ── 11. Verify the upgraded version ───────────────────────────────────────
    let actual_version = sys_info.version.as_deref().unwrap_or("unknown");
    tracing::info!(
        %pjh_id,
        %target_version,
        %actual_version,
        "execute_self_upgrade_host_job: agent reconnected"
    );

    // ── 12. Mark host job as succeeded ────────────────────────────────────────
    let output_msg = if install_output.is_empty() {
        format!(
            "Upgraded to {} (reported: {})",
            target_version, actual_version
        )
    } else {
        format!(
            "Upgraded to {} (reported: {})\n{}",
            target_version, actual_version, install_output
        )
    };
    ...
    .await
    {
        tracing::error!(%pjh_id, error = %e, "execute_self_upgrade_host_job: failed to mark pjh succeeded");
    }

    sync_job_status(&pool, job_id).await;
}
