...
async fn poll_host_health(
    ...
    let (
        ...
        Ok(client) => {
            let (status, payload, version, crl_status, crl_age, crl_next, gpg_status, gpg_expires) = match client
                .health()
                .await
            {
                Ok(data) => {
            ...
            (
                status, payload, version, sys_info, crl_status, crl_age, crl_next, gpg_status, gpg_expires,
            )
        },
    };
