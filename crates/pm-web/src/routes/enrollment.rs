...
async fn approve_enrollment(
    ...
    if let Some(existing_host) = sqlx::query_as::<_, Host>(
        "SELECT id, fqdn, ip_address::text, display_name, os_family, os_name, arch, agent_version, health_status, last_health_at, last_patch_at, agent_port, notes, registered_at, updated_at, crl_status, crl_age_seconds, crl_next_update, gpg_key_status, gpg_key_expires_at, COALESCE(pending_reboot, false) AS pending_reboot, COALESCE(reboot_paused, false) AS reboot_paused, COALESCE(package_db_clean, true) AS package_db_clean FROM hosts WHERE fqdn = $1 OR ip_address = $2::inet"
    )
